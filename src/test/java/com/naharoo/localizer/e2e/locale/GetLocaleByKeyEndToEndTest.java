...
class GetLocaleByKeyEndToEndTest extends AbstractEndToEndTest {

    @Autowired
    MockMvc mockMvc;

    @Test
    @DisplayName("Locale should be successfully found (200) when Locale with specified key exists")
    ...
    @Test
    @DisplayName("GetLocale should respond with 404 Not Found when Locale with provided key is not found")
    ...
    @Test
    @DisplayName("GetLocale should respond with 404 Not Found when Locale with provided key is deleted")
    void get_404_deleted() throws Exception {
        // Given
        final LocaleTestData testData = LocaleEndToEndTestHelper.getInitialData().get(3);
        final String key = testData.getId();
